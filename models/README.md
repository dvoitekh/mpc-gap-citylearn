...
| `nn_forecaster_mse.pt`, `nn_forecaster_dfl.pt`, `surrogate.pt`, `surrogate_data.npz` | see `experiments/dfl_surrogate.py` | decision-focused learning experiment |
...
```bash
python -m mpcgap.lgb_train --quick                            # -> models/lgb_models.pkl
...
hyperparameter search was run for the released models; `mpcgap/lgb_train.py` still
